...
import message.GroupUnreadMessage;
...
public class SGroupUnreadmsgHandler extends SimpleChannelInboundHandler<GroupUnreadMessage> {
    ...
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, GroupUnreadMessage groupUnreadMessage) throws Exception {
        try {

            //每次先打印一下下，看消息发过来没有！！！！！！
            System.out.println("打印消息"+groupUnreadMessage);


            //接受消息的部分
            int userid1 = groupUnreadMessage.getUserid();
            int groupid1= groupUnreadMessage.getGroupid();
            ...
            List<String> groupunreadmsg =new ArrayList<>();
            while (rs.next()) {
...
//                System.out.print(" ,发的消息是message: " + message);
                ...
                groupunreadmsg.add("发送者sendid: " + sendid1 + " ,接受者receiverid: " + receiver1 +
                        " ,发的消息是message: " + message  +  " ,聊天类型chattype:"+chattype + " ,消息类型message:" + messagetype);
